...
std::string truncStr(const std::string& input, size_t width) {
    if (input.length() > width) return input.substr(0, width - 1) + ".";
    return input;
}
...
void PhoneBook::searchContact() {
    ...
    for (int i = 0; i < contactCount; i++) {
        std::cout << std::setw(columnWidth) << i << "|"
                  ...
                  << truncStr(contacts[i].getFirstName(), columnWidth) << "|"
                  << std::setw(columnWidth)
                  << truncStr(contacts[i].getLastName(), columnWidth) << "|"
                  << std::setw(columnWidth)
                  << truncStr(contacts[i].getNickname(), columnWidth) << "\n";
    }
    ...
    if (index < 0 || index >= contactCount)
        ...
        std::cout << "\e[1mFirst name: \e[0m" << contacts[index].getFirstName()
                  ...
                  << "\n"
                  << "\e[1mPhone number: \e[0m"
                  << contacts[index].getPhoneNumber() << "\n"
                  << "\e[1mDarkest secret: \e[0m"
